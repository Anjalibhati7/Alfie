...
Tokens live in `apps/mobile/src/design/tokens.ts`. The screens use a system serif heading (Georgia on iOS/web, native serif on Android) and system body text; custom fonts remain a future design decision. Spacing follows 4/8/12/16/24/32/48 units, radii 0/4/8, and borders 1/2. Text scales with platform settings, content scrolls, and no animation runs by default.
